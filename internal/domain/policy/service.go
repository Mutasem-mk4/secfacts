...
func (Service) Compare(current []evidence.Finding, baseline []evidence.Finding) BaselineDiff {
	// Optimization: Store slice index instead of the struct by value to reduce memory overhead
	seenBaseline := make(map[evidence.Hash]int, len(baseline))
	// Optimization: Index-based iteration avoids expensive copying of the large Finding struct
	...
	for fingerprint, i := range seenBaseline {
